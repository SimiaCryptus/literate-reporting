/************************************************************************
 *
 * Just so you know, most of this code is Marijn's, via Ryan Boone.
 * from: Eloquent JavaScript by Marijn Haverbeke / Chapter 19 / Project: A Paint Program
 *
 * See Also:
...
function createPaint(parent, initialPaintUrl, width, height) {
  ...
  loadImageURL(cx, initialPaintUrl)
}
...
var painting_id = null
controls.save = function(cx) {
  // MUST open in a new window because of iframe security stuff
  var link = elt('button', {href: '/', target: '_blank'}, 'Save');
  function update() {
     try {
       console.log("Saving result");
         fetch("/input_" + painting_id + ".html", {
           method: "POST",
           body: cx.canvas.toDataURL()
         }).then(res => {
...
// initialize the app
function init(id, initialPaintUrl, width, height) {
  painting_id = id
  let timer = setInterval(() => {
    var paint_app = document.getElementById('paint-app')
    if (null != paint_app) {
        createPaint(paint_app, initialPaintUrl, width, height);
        clearInterval(timer);
    }
  }, 100);
 }
